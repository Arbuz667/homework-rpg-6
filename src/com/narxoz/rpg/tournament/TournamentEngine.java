...
public class TournamentEngine {
    ...
    public TournamentResult runTournament() {
        ...
        while (hero.isAlive() && opponent.isAlive() && round < maxRounds) {
            ...
            result.addLine("Hero actions: " + actionQueue.getCommandDescriptions().toString());
            actionQueue.executeAll();

            if (opponent.isAlive()) {
                result.addLine("Opponent attacks for " + opponent.getAttackPower());
                dodge.handle(opponent.getAttackPower(), hero);
            }

            result.addLine("Opponent HP: " + opponent.getHealth() + " | Hero HP: " + hero.getHealth());
        }
